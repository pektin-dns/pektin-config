...
export const checkConfig = async (path) => {
    ...
    if (config.nodes.filter(node => node.main !== true && typeof node.setup !== "object").length !==
        0)
        throw Error(`nodes that are not main must contain a setup object`);
    // nodes that are main cant contain a setup object
    if (config.nodes.filter(node => node.main === true && typeof node.setup !== "undefined")
        .length !== 0) {
    ...
    if (config.nameservers) {
        ...
        }
        {
            ...
            const allHaveMain = distinctDomains.every(d => {
                var _a;
                let hasMain = false;
                (_a = config.nameservers) === null || _a === void 0 ? void 0 : _a.forEach(ns => {
        ...
        }
        {
            ...
            if (Array.from(new Set(config.nameservers.map(ns => ns.subDomain + "." + ns.domain)))
